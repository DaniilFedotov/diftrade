...
def check_level(logger, version, current_price):
    ...
    for interval in intervals:  # Для каждого из временных промежутков
        ...
        if (max(highs) - DEF[interval]['hc'] * width < current_price < max(highs) + DEF[interval]['hc'] * width or
...
def sell_coin(logger, version, buy_info):
    ...
    params = {
        ...
        "recvWindow": 30000,  # Необходимо для предотвращения ошибки 1021
    }
    #print(params)
    ...
    stop_order_info = client.get_order(symbol="BTCTUSD", orderId=stop_order_id, recvWindow=30000)
    limit_order_info = client.get_order(symbol="BTCTUSD", orderId=limit_order_id, recvWindow=30000)
    ...
    while stop_order_status != 'FILLED' and limit_order_status != 'FILLED':
        ...
        time.sleep(timer)
        stop_order_info = client.get_order("BTCTUSD", orderId=stop_order_id, recvWindow=30000)
        limit_order_info = client.get_order("BTCTUSD", orderId=limit_order_id, recvWindow=30000)
        stop_order_status = stop_order_info['status']
        limit_order_status = limit_order_info['status']
    ...
    if stop_order_status == 'FILLED':
        ...
        message = f'{version}: Продано без стопа'
        logger.debug(message)
        send_message(logger, message)
    else:
        order_info = response
...
def get_balance(logger, version, tiker):
    ...
    all_tokens = client.account(recvWindow=30000)["balances"]
