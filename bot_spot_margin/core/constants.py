...
VLT_C = {  # VOLATILITY COEFFICIENTS, коэф-ты для проверки волатильности.
    ...
    3: {'hvc': 0.015,
        ...
        'end': 24},
    2: {'hvc': 0.01,
        'lvc': 0.0025,  # На 5-минутных таймфреймах добавить проверку волатильности, иначе в начале часа она недост
        'st': 22,
        'end': 24}
}
...
PRICE_DELTA_BTC = 0.5  # То, на сколько уменьшается цена покупки для выполнения ордера в режиме MAKER

BUY_ORDER_LIFETIME = 120000  # Время жизни лимитного ордера на покупку в милисекундах

TIMEDELTA_COMMISSION = 36000000  # Промежуток в милисекундах для отслеживания комиссии
