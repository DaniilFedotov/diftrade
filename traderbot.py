...
TRADER_VERSION = VERSIONS[0] # Выбрать версию бота
BINANCE_TOKEN = os.getenv('BINANCE_TOKEN')
TELEGRAM_TOKEN = os.getenv('TELEGRAM_TOKEN')
TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID')

TIME_STEP_CHECK = 4 # Выбрать частоту проверки в состоянии сделки
    ...
    while True:
        ...
        if current_price <= max(hash_list) * 0.9995:
            ...
            while deal_info['in_deal']:
                current_price = check_price()
                if current_price >= deal_info['purchase_price'] * 1.0005:
